...
Key improvements over Project 6:
  - Proper batching (32 sequences per update instead of 1)
  - Cosine annealing learning rate schedule
  - GELU activation instead of ReLU
  - 2000 epochs for deeper learning
...
# Batch all sequences as tensors for efficient training
sequences_tensor = torch.tensor(training_sequences).to(device)   # (499, 4)
targets_tensor   = torch.tensor(training_targets).to(device)     # (499,)
...
class TransformerBlock(nn.Module):
    ...
    Identical to Project 6 but now used as a repeating unit inside the LLM.
    Uses GELU activation instead of ReLU, which is standard in modern LLMs.
    """
    ...
    def __init__(self, embedding_dim, number_of_attention_heads,
        ...
        # GELU is smoother than ReLU and is the standard activation in GPT, BERT and modern LLMs
...
class MiniLanguageModel(nn.Module):
    ...
    Structure:
      ...
      2. Embedding dropout
      3. N stacked Transformer blocks (each with attention + FFN + residuals + layer norm)
    ...
    def __init__(self, vocabulary_size, embedding_dim, number_of_attention_heads,
        ...
        # Stack of Transformer blocks as a ModuleList
        # ModuleList registers all block parameters with the model automatically
    ...
    def _build_causal_mask(self, sequence_length, device):
        return torch.triu(
...
# Cosine annealing: gradually reduces learning rate to near zero by the end of training
# Prevents overshooting the minimum in later epochs
...
for epoch in range(number_of_epochs):
    ...
    if epoch % 400 == 0:
        ...
        print(f"Epoch {epoch:5d}  loss: {average_epoch_loss:.4f}  lr: {current_lr:.6f}")
...
def generate_text(seed_words, number_of_words_to_generate=8, temperature=1.0):
    ...
    temperature controls randomness:
      temperature = 0.0  greedy, always picks the most likely word
      temperature = 0.8  slightly creative, mostly coherent
      temperature = 1.0  standard sampling
      temperature = 1.5  creative but may lose coherence
    """
    model.eval()
